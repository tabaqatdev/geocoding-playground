...
export function Playground() {
  ...
  const { sdk, error, stats, initialized, searchMode, retry } = useGeoSDK();
  ...
  const handleMapClick = useCallback(
    async (e: MapLayerMouseEvent) => {
      ...
      try {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const country = countryResult as any;
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const hierarchy = hierarchyResult as any;

        setAdminHierarchy({
          country: language === "ar" ? country?.name_ar : country?.name_en,
          region: language === "ar" ? hierarchy?.region?.name_ar : hierarchy?.region?.name_en,
          governorate:
            language === "ar" ? hierarchy?.governorate?.name_ar : hierarchy?.governorate?.name_en,
          district: language === "ar" ? hierarchy?.district?.name_ar : hierarchy?.district?.name_en,
        });
      } catch (err) {
                  ...
                  <TabsContent value="reverse" className="m-0 p-4 data-[state=active]:block">
                    <div className="space-y-3">
                      ...
                      <Button
                        ...
                        className="w-full"
                      >
                        {reverseLoading ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : null}
                        {t("common.search")}
                      </Button>
